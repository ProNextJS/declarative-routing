...
| `/` | - | `Home` | `<Home.Link>` |
| `/pokemon/[_id]` | - | `PokemonId` | `<PokemonId.Link>` |
...
- [ ] `/pokemon/[_id]/page.info.ts`: Add search typing to if the page supports search paramaters
- [ ] Convert `Link` components for `/pokemon/[_id]` to `<PokemonId.Link>`
- [ ] Convert `params` typing in `/pokemon/[_id]/page.ts` to `z.infer<>`
